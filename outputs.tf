...
output "storage_queues_resource_manager_id" {
  ...
  value       = { for k, v in azurerm_storage_queue.storage_queues : k => v.resource_manager_id if v.resource_manager_id != null && length(v.resource_manager_id) > 0 }
}
...
output "storage_queues_storage_account_name" {
  ...
  value       = { for k, v in azurerm_storage_queue.storage_queues : k => v.storage_account_name if v.storage_account_name != null && length(v.storage_account_name) > 0 }
}
